...
for d1 in os.listdir(root):
    ...
    for f in rgb_files:
        ...
        match = os.path.join(d_path, tag + '_depth_vi.png')import os
        from zipfile import ZipFile
        import shutil
        ...
        print("Unzipping testing files...")
        with ZipFile(os.path.join(curdir, 'indoor_test.zip'), 'r') as zip1:
            zip1.extractall(root)

        print("Unzipping training files...")
        with ZipFile(os.path.join(curdir, 'indoor_train.zip'), 'r') as zip2:
            zip2.extractall(root)
